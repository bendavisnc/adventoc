...
(def hallway-size
  (case (count rooms)
    2 7
    4 11
    (throw (ex-info (str "Unknown hallway size for " (count rooms) " rooms")
                    {:rooms rooms}))))
...
(defn graph-rows-columns-switch
  [graph]
...
(defn str->burrow
  ...
  (let [[amphipods-acc _]
        ...
        burrow         (case (count rooms)
                         ...
                         (-> burrow-empty
                             (assoc-in [:room :A]
                                       (vec (rseq (amphipod-graph 0))))
                             (assoc-in [:room :B]
                                       (vec (rseq (amphipod-graph 1)))))
                         4
                         (->
                           burrow-empty
...
(defn energy
  ...
  (if-let [e (some-> amphipod
                     ...
                     energys)]
    e
    (throw (ex-info (str "Unknown amphipod energy for : " amphipod)
                    {:amphipod amphipod}))))
...
(defn move-most-recent
  [amphipod moves]
  (or (some (fn [move]
              (when (= amphipod (first move))
                move))
            (rseq moves))
      (throw (ex-info (str "Amphipod `" amphipod "` has no moves.")
                      {:amphipod amphipod
                       :moves    moves}))))
    ...
    (reduce (fn [acc amphipod]
              (let [last-move     (some (fn [move]
                ...
                (try
                  ...
                  (catch Exception e
                    (throw (ex-info (str "Error placing amphipod " amphipod
                                         " at position " move-position)
                                    {:amphipod      amphipod
...
(defn can-move?
  ...
  (let [burrow (:burrow journey)
    ...
    (cond
      ...
      (not (= #{nil}
              (set (for [p (positions-between current-position
                                              position)]
                     (get-in burrow p)))))
      false
      :else
...
(defn goal
  ...
  (let [burrow         (:burrow journey)
        ...
        amphipods-all-home? (= (case (count rooms)
                                 ...
                                 [#{:A} #{:B} #{:C} #{:D}])
                               (for [room rooms]
                                 (set (map amphipod->room
                                           (get-in burrow [:room room])))))]
...
(def distance
  (memoize
   (fn [from to]
     (assert (#{:room :hallway} (first from))
             (str "Invalid `from` position: " from))
     ...
     (case [(first from) (first to)]
       ...
       (let [[_ room-from room-from-index] from
         ...
         (if (= room-from room-to)
           ...
           (+ from-room-distance
              1 to-room-distance
              1 (distance [:hallway hallway-from-room-index]
                          [:hallway hallway-to-room-index]))))
       (throw (ex-info (str "Unknown distance for from " from " to " to)
                       {:from from :to to}))))))
...
(defn move-applied
  ...
  (let [[amphipod move-position] move
        ...
        move-position-start (second (move-most-recent amphipod
                                                      (:moves journey)))
        _ (assert move-position-start
                  (str "Amphipod "
                       amphipod
                       " has no starting position in journey moves."))
...
(defn journeys-afresh
  ...
  (for [amphipod       amphipods
        ...
        :let           [journey-next (-> journey
                                         (move-applied [amphipod
                                                        move-placement])
                                         (assoc-burrow))]]
    journey-next))

(defn journey->breadcrumbs
  [journey]
