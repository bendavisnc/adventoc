...
(defn input->grid-keep-whitespace
  [input]
  (vec (for [row (vec (string/split-lines input))]
         (vec row))))

(defn grid-rows-columns-switch
  [grid]
...
(defn process-step
  ...
  (m/match (merge acc
    ...
    (merge acc
           {:operator ?op
            :operands (if-let [operand (column->operand (reverse (rest
                                                                  column)))]
...
(defn process-math-ops-right->left
  [grid]
  (let [s       {:operands nil
                 :operator nil
                 :results  nil}
        columns (concat (reverse (map reverse grid)) ;; empty list to add
                                                     ;; one last step for
                                                     ;; performing the last
                                                     ;; operation.
                        (list []))]
...
(defn trashcompactor
  ([input {:keys [right-to-left]}]
   (let [sums
         (if right-to-left
           (process-math-ops-right->left (grid-rows-columns-switch
                                          (input->grid-keep-whitespace input)))
